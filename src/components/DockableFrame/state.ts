...
export function addNewContent(state: State, toPanel: Panel, element: ContentElement) {
    const id = state.idNext++;
    toPanel.contentList.push({ contentId: id, title: "", element });
...
export function dock(state: State, panel: Panel, dockIntoPanel: Panel, mode: DockMode) {
    if (mode == DockMode.Full || (dockIntoPanel.contentList.length == 0 && dockIntoPanel.splitPanels.length == 0)) {
    ...
    } else if (mode == DockMode.Right || mode == DockMode.Left || mode == DockMode.Top || mode == DockMode.Bottom) {
        const subdivMode = (mode == DockMode.Right || mode == DockMode.Left) ? SplitMode.LeftRight : SplitMode.TopBottom;
...
export function clampFloatingPanels(state: State, rect: Rect) {
    ...
    for (const panel of state.floatingPanels) {
        panel.rect.x = Math.max(rect.x + margin - panel.rect.w / 2, Math.min(rect.x2 - margin - panel.rect.w / 2, panel.rect.x));
        panel.rect.y = Math.max(rect.y + margin, Math.min(rect.y2 - margin - panel.rect.h / 2, panel.rect.y));
    }
}
...
export function clampFloatingPanelStrictly(state: State, panel: Panel, rect: Rect) {
    const margin = 10;
    panel.rect.x = Math.max(rect.x + margin, Math.min(rect.x2 - margin - panel.rect.w, panel.rect.x));
    panel.rect.y = Math.max(rect.y + margin, Math.min(rect.y2 - margin - panel.rect.h, panel.rect.y));
}
...
export function traverseLayout(panel: Panel, rect: Rect, layout: Layout) {
    ...
    if (panel.splitPanels.length == 2) {
        if (panel.splitMode == SplitMode.LeftRight) {
            ...
            layout.dividers.push({ panel, vertical: false, rect: rectDivider, resizeMin: rect.x1, resizeMax: rect.x2 });
            ...
            layout.dividers.push({ panel, vertical: true, rect: rectDivider, resizeMin: rect.y1, resizeMax: rect.y2 });
        }
    } else {
        const panelRect = { panel, rect, floating: false, zIndex: 0 };
        for (let w = 0; w < panel.contentList.length; w++) {
            layout.content.push({ content: panel.contentList[w], tabIndex: w, panel, layoutPanel: panelRect });
        }
        layout.panelRects.push(panelRect);
        layout.anchors.push({ panel, x: xMid, y: yMid, mode: DockMode.Full, previewRect: rect });
    }

    layout.anchors.push({ panel, x: rect.x2 - 10, y: yMid, mode: DockMode.Right, previewRect: rect.withX1(rect.x1 + (rect.x2 - rect.x1) * 3 / 4) });
    layout.anchors.push({ panel, x: rect.x1 + 10, y: yMid, mode: DockMode.Left, previewRect: rect.withX2(rect.x1 + (rect.x2 - rect.x1) / 4) });
    layout.anchors.push({ panel, x: xMid, y: rect.y2 - 10, mode: DockMode.Bottom, previewRect: rect.withY1(rect.y1 + (rect.y2 - rect.y1) * 3 / 4) });
    layout.anchors.push({ panel, x: xMid, y: rect.y1 + 10, mode: DockMode.Top, previewRect: rect.withY2(rect.y1 + (rect.y2 - rect.y1) / 4) });
}
