        ...
        {links[0]?.links ? abc.map((a,i) => {
            ...
            return (
                <details key={`link-${i}`} open>
                    ...
                    {group && group.links && group.links.length ? group.links.map((link,index) => <LinkCmp key={index} link={link} />): <div>No data</div>}
                </details>
            );
        }) : links.map((link, index) => <LinkCmp key={index} link={link} />)}
    </Fragment>
);
